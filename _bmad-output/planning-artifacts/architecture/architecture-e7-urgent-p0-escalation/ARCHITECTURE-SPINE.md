...
- **AD-5 / AD-6:** the P0 rule *shape* is decided (a configurable OR-of-AND multi-field expression); what remains is the org's **default values** — which fields + thresholds populate that rule (priority/urgency/impact/category) — and confirming their GLPI field IDs via the discovery probe before E7-S2.
